...
export class ServiceApresVenteController {
  ...
  async updateServiceApresVente(request: Request, response: Response) {
    try {
      const id = parseInt(request.params.id);
      ...
      await this.serviceApresVenteRepository.save(serviceApresVente);
      response.json({ message: "Service après-vente mis à jour", serviceApresVente });
    } catch (error) {
      console.error("Erreur lors de la mise à jour du service après-vente:", error);
      response.status(500).json({ message: "Erreur lors de la mise à jour du service après-vente", error });
    }
  }

  async deleteServiceApresVente(request: Request, response: Response) {
    try {
      const id = parseInt(request.params.id);
      const serviceApresVente = await this.serviceApresVenteRepository.findOne({ where: { id } });
      if (!serviceApresVente) {
      ...
      response.json({ message: "Service après-vente supprimé" });
    } catch (error) {
      console.error("Erreur lors de la suppression du service après-vente:", error);
      response.status(500).json({ message: "Erreur lors de la suppression du service après-vente", error });
    }
  }
